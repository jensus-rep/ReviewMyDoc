...
function init(root) {
  ...
  function showCollection(result) {
    ...
    list.replaceChildren(...result.passages.map((p) => {
  ...
  async function collect(assign) {
    ...
    try {
      ...
      report("Passage gespeichert und f\xFCr das Review gesammelt");
  ...
  toolbar.querySelector("[data-assign]").addEventListener("click", () => {
    void collect(true);
  });
}
